...
var topFeedback = document.getElementById("message-feedback-top");
var messageForm = document.getElementById("message-form");
var messageInput = document.getElementById("message-input");
var setQuestionBtn = document.getElementById("setQuestionBtn");
var answerInput = document.getElementById("answerInput");
var questionInput = document.getElementById("questionInput");
var submitQA = document.getElementById("submitQA");
var closePopover = document.getElementById("closePopover");
var leave = document.getElementById("leave");
var timer = document.getElementById("timeLeft");
var scores = document.getElementById("playerScores");
var code = document.getElementById("code");
var lobbyScreen = document.getElementById("lobbyScreen");
var gameScreen = document.getElementById("gameScreen");
var messageTone = new Audio("/answer.wav");
...
if (messageContainer && messageForm) {
  ...
  socket.on("session_created", function (data) {
    console.log(data);
    username.innerText = "".concat(data);
    //clearTopFeedback();
  ...
  socket.on("guess_error", function (data) {
    (0, _alert.showAlert)("error", data);
    console.log(data);
  });
  // };
  socket.on("code", function (data) {
    code.textContent = "Game code: ".concat(data);
  });
  ...
  socket.on("attempted", function (data) {
    (0, _alert.showAlert)("success", "you've just ".concat(data, " retry left"));
  });
  socket.on("question_created", function (question) {
    messageTone.play();
  ...
  var addMessageToUI = function addMessageToUI(isOwnedMessage, data) {
    var element = "\n      <li class=\"".concat(isOwnedMessage ? "message-right" : "message-left", "\">\n        <p class=\"message\">\n          ").concat(isOwnedMessage ? data.guess : data, "\n          <span> \u2022 ").concat(moment(data.datetime).fromNow(), "</span>\n        </p>\n      </li>");
    messageContainer.innerHTML += element;
    messageContainer.scrollTo(0, messageContainer.scrollHeight);
  };
  socket.on("players_scores", function (data) {
    scores.innerHTML = "";
    data.forEach(function (player) {
      var playerItem = document.getElementById("score-".concat(player._id));
      if (!playerItem) {
        playerItem = document.createElement("li");
        playerItem.id = "score-".concat(player._id);
        scores.appendChild(playerItem);
      }
      playerItem.textContent = "".concat(player.username, ": ").concat(player.score);
    });
  });
  var players = function players(data) {
    var element = "\n     <li class=\"playerscores\" id=\"playerScores\">\n      <p>".concat(data, ": 10 </p>\n     </li>\n    ");
    scores + element, _readOnlyError("scores");
  };
  var feedbackTop = function feedbackTop(is_host, data) {
    var element = "\n      <li class=\"message-feedback-top\">\n        <p class=\"feedback-top\">".concat(data, " ").concat(is_host ? "is the host" : "joined", "</p>\n      </li>");
    topFeedback.innerHTML += element;
  };
  var feedback = function feedback(is_restarted, data) {
    var element = "\n      <li class=\"message-feedback\">\n        <p class=\"feedback\">".concat(data, " ").concat(is_restarted ? "" : "left", "</p>\n      </li>");
    messageContainer.innerHTML += element;
  };
}
...
if (setQuestionBtn) {
  ...
  socket.on("assignGameMaster", function (gm) {
    var myUserId = window.currentUserId;
    if (gm === myUserId) {
      setQuestionBtn.style.display = "block"; // I’m the GM
    } else {
      setQuestionBtn.style.display = "none"; // Not me
    }
    //setQuestionBtn.style.display = isGameMaster ? "block" : "none";
  });
}
if (submitQA) {
  submitQA.addEventListener("click", function () {
...
if (leave) {
  leave.addEventListener("click", function () {
    ...
    socket.emit("leave-game", function (res) {
